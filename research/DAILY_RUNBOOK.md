...
9. Run `npm run wildlife:refresh` to update both per-creature sightings and the end-of-directory iNaturalist species-atlas counts. Commit generated files only when evidence actually changes. Re-check population claims and wildlife distances against NOAA, USFWS, Hawaiʻi DLNR, or University of Hawaiʻi sources; preserve photo credit, license, and original record whenever a photo changes.
...
- **7:00am · notable-sightings alert:** `scripts/cron-notable-sightings.sh` (wraps `scripts/check-notable-sightings.mjs`) scans recent animal observations within ~60km of Mana Kai for a curated watchlist plus a rarity/threatened/endemic fallback. It excludes plants and introduced fallback taxa, labels tentative IDs, ranks research-grade and watchlisted finds first, keeps one record per species, and cools down repeat taxa so the alert stays diverse. One top find may carry a compact expert field note, research link, and ethical experience. It never edits site data, commits, or pushes; dedup and cooldown state live in the gitignored `research/.notable-sightings-seen.json`.
