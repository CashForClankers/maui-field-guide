...
- Wildlife community-sighting evidence (`src/data/sightings.ts`) is generated by `scripts/fetch-inaturalist-sightings.mjs` — never hand-edit it; regenerate and commit only when the data actually changed.
- The end-of-directory species atlas (`src/data/inaturalistDiscovery.ts`) is generated by `scripts/fetch-inaturalist-discovery.mjs`; refresh it with the other wildlife evidence and keep its links species-first, research-grade, non-captive, and free of republished observation coordinates.
- The notable-sightings Telegram alert (`scripts/check-notable-sightings.mjs`, classification logic in `scripts/lib/notable-sightings.mjs`) is a separate, read-only-to-the-site lane. It never touches published data, never commits, and stays silent when nothing new and genuinely notable is found. Any change to its watchlist or rarity thresholds needs a matching test in `tests/notable-sightings.test.ts`.
